...
def get_blog_list_common_data(request, blog_all_list):
    ...
    # 获取当前页码前后各两页范围
    page_range = list(range(max(current_page_number-2, 1), current_page_number)) + list(range(current_page_number, min(current_page_number+2, paginator.num_pages)+1))
    # 加上省略页标记
    if page_range[0] - 1 >= 2:
        page_range.insert(0, '...')
...
def blog_detail(request, blog_pk):
    ...
    context = {}
    context['pervious_blog'] = Blog.objects.filter(created_time__gt=blog.created_time).last()
